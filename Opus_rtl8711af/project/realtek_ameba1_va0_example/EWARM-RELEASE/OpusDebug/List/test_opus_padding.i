...
int test_overflow(void)
{
  ...
  unsigned char *in = malloc(16909318);
  opus_int16 *out = malloc(5760*2*sizeof(*out));

  fprintf((& __iar_Stderr), "  Checking for padding overflow... ");
  if (!in || !out) {
    fprintf((& __iar_Stderr), "FAIL (out of memory)\n");
    return -1;
  }
  in[0] = 0xff;
  in[1] = 0x41;
  memset(in + 2, 0xff, 16909318 - 3);
  in[16909318-1] = 0x0b;

  decoder = opus_decoder_create(48000, 2, &error);
  result = opus_decode(decoder, in, 16909318, out, 5760, 0);
  ...
  if (result != -4) {
    ...
    _test_failed("C:\\Users\\PCCUBE\\Desktop\\Opus_rtl8711af\\external\\opus\\tests\\test_opus_padding.c", 70);;
  }

  fprintf((& __iar_Stderr), "OK.\n");
...
int main(void)
{
  ...
  if (!oversion) _test_failed("C:\\Users\\PCCUBE\\Desktop\\Opus_rtl8711af\\external\\opus\\tests\\test_opus_padding.c", 85);;
  fprintf((& __iar_Stderr), "Testing %s padding.\n", oversion);

  tests += test_overflow();

  fprintf((& __iar_Stderr), "All padding tests passed.\n");
