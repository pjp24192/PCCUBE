...
;(function () {
	...
	function popup_open( popup ) {
		$.ajax({                                      
			url: '/wp-content/plugins/wordpress-popup/js/get_terms_val.php',       //the script to call to get data          
			data: "",
			dataType: 'text',               //data format      
			success: function(data) {       //on recieve of reply
				var agree = data; 
				if(agree.indexOf('YES') !== -1)
					agree = 'YES';
				else
					agree = 'NO';
				//alert(agree);
				...
				if ( 'closed' === po_status ) {
					...
					po_current = popup;
					po_status = 'open';
					$('.wdpu-cta').click(function(){
						var clickBtnValue = $(this).val();
						var ajaxurl = '/wp-content/plugins/wordpress-popup/js/btn_action.php',
		...
		/*if ( 'closed' === po_status ) {
			if ( popup._show() ) {
				...
				$('.wdpu-cta').click(function(){
					var clickBtnValue = $(this).val();
					var ajaxurl = '/wp/wp-content/plugins/wordpress-popup/js/btn_action.php',
					data =  {'action': clickBtnValue};
					$.post(ajaxurl, data, function (response) {
...
/*$(document).ready(function(){
    $('#popup_btn').click(function(){
        var clickBtnValue = $(this).val();
        alert("btn action performed successfully" + clickBtnValue);
        var ajaxurl = 'btn_action.php',
        data =  {'action': clickBtnValue};
        $.post(ajaxurl, data, function (response) {
            // Response div goes here.
            alert("btn action performed successfully");
        });
    });
});*/
